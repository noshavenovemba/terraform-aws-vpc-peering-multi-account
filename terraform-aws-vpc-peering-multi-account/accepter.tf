...
locals {
    ...
    accepter_aws_rt_map = { for s in local. accepter_subnet_ids : S => try(data.aws_route_tables.accepter [s]}
...
resource "aws_route" "accepter" {
    count = local.enabled ? local.accepter_aws_route_table_ids_count * local.requester_cidr_block_associate
    provider = aws.accepter
    route_table_id = local.accepter_aws_route_table_ids[floor(count.index / local.requester_cidr_block_associations]
    destination_cidr_block = local.requester_cidr_block_associations[count.index % local.requester_cidr_block_associations_]
    vpc_peering_connection_id = join("'", aws_vpc_peering_connection.requester.*.id)
    depends_on = [
        data.aws_route_tables.accepter, 
        aws_vpc_peering_connection_accepter.accepter, 
        aws_pc_peering_connection.requester,
    ]
    timeouts {
...
resource "aws_vpc_peering_connection_accepter" "accepter" {
    ...
    tags = merge (
        ...
        {
            Name = var.accepter_pcx_name
        }
    )
}

resource "aws_pc_peering_connection_options" "accepter" {
    count = 0
    provider = aws.accepter
