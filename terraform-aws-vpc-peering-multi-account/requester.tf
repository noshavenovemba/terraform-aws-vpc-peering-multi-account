...
variable "requester_aws_secret_key" {
    description = "Secret access key to use in requester"
    type = string
    default = null    
}

variable "requester_aws_token" {
    description = "Session token for validating temporary"
    type = string
    default = null    
}
...
variable "requester_allow_remote_vpc_dns_resolution" {
    type = bool 
    default = true
    description = "Allow requester VPC to resolve public DNS hostnames to private IP addresses when queried from instances"
}

variable "requester_pcx_name" {
    type = string
}
...
module "requester" {
    ...
    tag = var.add_attribute_tag ? { Side = "requester"} : {}
...
data "aws_route_table" "requester" f
...
resource "aws_vpc_peering_connection" "requester" {
    ...
    auto_accept = false
    tags = merge (
        module.requester.tags,
        {
            Name = var.requester_pcx_name
        }
    )
}
...
resource "aws_pc_peering_connection_options" "requester" {
...
resource "aws_route" "requester" {
    count = local.enabled ? local.requester_aws_route_table_ids_count * local.accepter_cidr_block_associations_
    provider = aws.requester
    route_table_id = local.requester_aws_route_table_ids[floor(count.index / local. accepter_cidr_block_associations_coun
    destination_cidr_block = local.accepter_cidr_block_associations[count.index % local.accepter_cidr_block_associations_count)l
